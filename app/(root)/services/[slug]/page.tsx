...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const { data: sanityService } = await sanityFetch({
  ...
  const ogImage = sanityService?.image?.asset?.url || staticService?.image || "https://divitmindspace.com/divit-mindspace-logo.png";
...
export default async function ServicePage({ params }: PageProps) {
  ...
  // Use sanity service if exists, otherwise construct from static data
  const service: Partial<ServiceData> = sanityService || {
    ...
    benefits: staticService?.content.benefits,
    whatToExpect: staticService?.content.whatToExpect,
    whoIsItFor: staticService?.content.whoIsItFor,
    duration: staticService?.content.duration,
    format: staticService?.content.format,
  };
  ...
  const jsonLd = {
    ...
    "provider": {
      ...
      "url": SITE_URL,
      "logo": `${SITE_URL}/divit-mindspace-logo.png`,
      "address": {
        "@type": "PostalAddress",
  ...
  const categoryColors: Record<string, string> = {
    assessments: "var(--color-assessments)",
    therapy: "var(--color-therapy)",
    guidance: "var(--color-guidance)",
    programs: "var(--color-programs)",
    physiotherapy: "var(--color-physio)",
  };

  const categoryColor = categoryColors[service.category] || "var(--color-assessments)";
      ...
      <section className="pt-0 pb-2 lg:pt-0 lg:pb-4">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <h1
              className="text-3xl md:text-4xl lg:text-5xl font-serif text-green mt-1 lg:mt-2 mb-1 lg:mb-2 leading-tight relative inline-block"
              style={{ fontFamily: "'Cormorant Garamond', 'Georgia', serif" }}
            >
              {service.title}
              <svg className="absolute -bottom-2 left-0 w-full h-3 text-green/20" viewBox="0 0 200 20" preserveAspectRatio="none">
                <path d="M5 15 Q 50 5, 100 15 T 195 15" stroke="currentColor" strokeWidth="4" fill="transparent" strokeLinecap="round" />
              </svg>
            </h1>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {service.overview && (
              <div className="mb-6 lg:mb-8">
                ...
                <p className="text-black/70 text-base lg:text-lg leading-relaxed font-medium italic whitespace-pre-wrap text-editorial-dropcap">
                  {service.overview}
                </p>
              </div>
            )}
              ...
              <div className="bg-white rounded-2xl pt-4 pb-5 px-5 lg:pt-6 lg:pb-8 lg:px-8 border border-green/10 shadow-sm mb-12">
                ...
                <div className="flex flex-col lg:flex-row gap-6 mb-8">
                  ...
                  <div className="w-full lg:w-72 shrink-0">
                    {service.description && (
                      <div 
                        className="p-5 lg:p-6 rounded-xl text-black shadow-lg relative overflow-hidden group transition-all"
                        style={{ backgroundColor: categoryColor }}
                      >
                        <div className="absolute -right-2 -bottom-2 opacity-10 group-hover:scale-110 transition-transform duration-500">
                          <CheckCircle2 className="w-20 h-20" />
                        </div>
                        <p className="text-[9px] font-bold uppercase tracking-[0.2em] mb-3 opacity-40">Primary Outcome</p>
                        <p className="text-lg font-serif italic leading-snug font-bold relative z-10 text-black/80">
                          {service.description}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {service.benefits.map((benefit, i) => (
                        <div key={i} className="h-full p-5 lg:p-6 rounded-xl bg-white border border-green/5 shadow-sm hover:shadow-md hover:border-green/20 transition-all group flex flex-col items-start text-left">
                          <div 
                            className="w-9 h-9 rounded-full flex items-center justify-center mb-4 transition-all duration-300"
                            style={{ backgroundColor: `${categoryColor}40` }} // 40 is ~25% opacity
                          >
      ...
      <section className="pt-2 pb-2 lg:pt-4 lg:pb-4 bg-[#FAF9F5]">
        <div className="container mx-auto px-4">
          <InlineCtaBlock 
            ...
            whatsappMessage={`Hi Divit MindSpace, I'm interested in learning more about ${service.title}.`}
          />
        </div>
      </section>
    </div>
