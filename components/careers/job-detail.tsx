"use client";

import { PortableText } from "next-sanity";
import { portableTextComponents } from "@/components/portable-text-components";
import { careerPortableTextComponents } from "./career-portable-text";
import type { Career } from "@/sanity/types";
import { WhatsAppConsultationLink } from "@/components/whatsapp-consultation-link";

interface JobDetailProps {
  job: Career;
}

export function JobDetail({ job }: JobDetailProps) {
  const headingStyle = "text-2xl font-serif italic font-bold text-green mb-6";
  const headingFont = { fontFamily: "'Cormorant Garamond', 'Georgia', serif" };

  const whatsappMessage = `Hi, I'm interested in applying for the ${job.title} position at Divit MindSpace.`;

  return (
    <div className="bg-[#FDFBF7] min-h-screen">
      <div className="container mx-auto px-4 py-8 md:px-6 lg:px-8">
        {/* Job header */}
        <div className="mb-8">
          <div className="mb-2">
            <span className="text-sm font-semibold text-green/60 uppercase tracking-wide">
              Careers
            </span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold text-green mb-4">
            {job.title}
          </h1>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Left sidebar */}
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl p-6 border border-green/10 sticky top-8">
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-semibold text-green/60 mb-2">Location</h3>
                  <p className="text-green capitalize">{job.location.join(', ')}</p>
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-green/60 mb-2">Employment Type</h3>
                  <p className="text-green capitalize">{job.employmentType.replace('-', ' ')}</p>
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-green/60 mb-2">Location Type</h3>
                  <p className="text-green capitalize">{job.locationType}</p>
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-green/60 mb-2">Department</h3>
                  <p className="text-green">{job.department}</p>
                </div>
              </div>
            </div>
          </div>

          {/* Right content */}
          <div className="lg:col-span-2">
            <div className="space-y-10">
              {/* 1. Intro / About this role */}
              <div>
                <h2 className={headingStyle} style={headingFont}>
                  About this role
                </h2>
                <div className="max-w-none text-green/80 text-lg leading-relaxed font-medium">
                  <PortableText
                    value={job.aboutRole}
                    components={portableTextComponents}
                  />
                </div>
              </div>

              {/* 2. Key Responsibilities */}
              {job.responsibilities && job.responsibilities.length > 0 && (
                <div>
                  <h2 className={headingStyle} style={headingFont}>
                    Key Responsibilities
                  </h2>
                  <ul className="space-y-4">
                    {job.responsibilities.map((resp, idx) => (
                      <li key={idx} className="flex items-start group">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#7A9A7D] mr-4 mt-2.5 shrink-0 opacity-60 group-hover:opacity-100 transition-opacity" />
                        <span className="text-green/80 text-base leading-relaxed">{resp}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              )}

              {/* 3. Requirements */}
              {job.requirements && job.requirements.length > 0 && (
                <div>
                  <h2 className={headingStyle} style={headingFont}>
                    Requirements
                  </h2>
                  <ul className="space-y-4">
                    {job.requirements.map((req, idx) => (
                      <li key={idx} className="flex items-start group">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#7A9A7D] mr-4 mt-2.5 shrink-0 opacity-60 group-hover:opacity-100 transition-opacity" />
                        <span className="text-green/80 text-base leading-relaxed">{req}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              )}

              {/* 4. Additional Advantage */}
              {job.additionalAdvantage && job.additionalAdvantage.length > 0 && (
                <div>
                  <h2 className={headingStyle} style={headingFont}>
                    Additional Advantage
                  </h2>
                  <div className="max-w-none text-green/80 text-base leading-relaxed">
                    <PortableText
                      value={job.additionalAdvantage}
                      components={careerPortableTextComponents}
                    />
                  </div>
                </div>
              )}

              {/* 5. What We Offer */}
              {job.whatWeOffer && job.whatWeOffer.length > 0 && (
                <div>
                  <h2 className={headingStyle} style={headingFont}>
                    What We Offer
                  </h2>
                  <div className="max-w-none text-green/80 text-base leading-relaxed">
                    <PortableText
                      value={job.whatWeOffer}
                      components={careerPortableTextComponents}
                    />
                  </div>
                </div>
              )}

              {/* Apply button */}
              <div className="pt-6">
                <WhatsAppConsultationLink 
                  message={whatsappMessage}
                  source="career_apply_bottom"
                  className="bg-green hover:bg-green/90 text-white px-8 py-6 text-lg rounded-lg font-bold transition-all duration-300 shadow-xl shadow-green/20 flex items-center justify-center gap-3 w-full sm:w-auto"
                >
                  <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                  Apply Now
                </WhatsAppConsultationLink>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
