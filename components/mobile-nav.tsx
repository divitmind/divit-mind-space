              ...
              <AccordionItem value="services" className="border-none">
                ...
                <AccordionContent className="flex flex-col gap-2 p-4 pt-2">
                  <div className="grid grid-cols-2 xs:grid-cols-3 gap-2 mb-4">
                    {serviceCategories.map((group) => (
                      <MobileLink
                        ...
                        href={group.href}
                        setOpen={setOpen}
                        icon={<group.icon className="h-3.5 w-3.5" />}
                        className="px-3 py-3 rounded-xl bg-white border border-black/5 text-[9px] font-bold uppercase tracking-widest text-black/50 flex-col items-center text-center gap-2"
                      >
                        {group.label}
                      </MobileLink>
                    ))}
                  ...
                  <div className="space-y-1 border-t border-black/5 pt-4">
                    <p className="px-4 pb-2 text-[8px] font-bold uppercase tracking-[0.2em] text-black/20">Popular Services</p>
                    {topServices.map((service) => (
                      <Link
                        key={service.id}
                        href={`/services/${service.slug}`}
                        onClick={() => setOpen(false)}
                        className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-green/5 group transition-colors"
                      >
                        <div className="w-1 h-1 rounded-full bg-green/20 group-hover:bg-green transition-colors" />
                        <span className="text-xs font-medium text-black/70 group-hover:text-green transition-colors">{service.title}</span>
                      </Link>
                    ))}
                  </div>
