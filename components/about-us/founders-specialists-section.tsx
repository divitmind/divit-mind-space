...
function PersonCard({ person }: { person: Specialist }) {
  return (
    <article className="flex flex-col rounded-3xl overflow-hidden h-full group bg-white shadow-sm hover:shadow-md transition-shadow duration-300">
      {/* Image */}
      <div className="relative w-full aspect-square rounded-3xl shrink-0 overflow-hidden">
        ...
        {person.experience && (
          <div className="absolute top-3 right-3 bg-white/95 backdrop-blur-sm px-2.5 py-1 rounded-full shadow-sm">
            <span className="text-xs font-bold text-[#004540]">{person.experience}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 p-5 sm:p-6">
        <h3 className="text-xl font-bold text-[#2F3E33] tracking-tight leading-tight">
          {person.name}
        </h3>
        
        {/* Credentials / Title - Visible by default */}
        <div className="mt-2.5">
          <p className="text-sm font-semibold text-[#004540] leading-relaxed italic">
            {person.title}
          </p>
        </div>
        ...
        <div className="mt-auto pt-6 flex flex-wrap items-center justify-between gap-4">
          <Dialog>
            <DialogTrigger
              className={cn(
                ...
                "text-[#2F3E33] font-bold text-sm",
                "bg-[#FDFBF7] border border-[#2F3E33]/10 px-4 py-2 rounded-full",
                "hover:bg-[#2F3E33] hover:text-white transition-all duration-300",
                "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2F3E33] focus-visible:ring-offset-2"
              )}
            >
              Bio +
            </DialogTrigger>
            <DialogContent
              className="sm:max-w-[640px] max-h-[90vh] overflow-hidden flex flex-col rounded-[2.5rem] bg-[#FDFBF7] border-none shadow-2xl p-0"
              showCloseButton
            >
              <div className="overflow-y-auto flex-1 px-6 sm:px-10 py-10 sm:py-12">
                <DialogHeader className="mb-8">
                  <DialogTitle className="text-2xl sm:text-3xl font-bold text-[#2F3E33] mb-3">
                    {person.name}
                  </DialogTitle>
                  <p className="text-base font-semibold text-[#004540] leading-relaxed italic">
                    {person.title}
                  </p>
                </DialogHeader>
                
                <div className="space-y-6 text-[#2F3E33]/90 text-[15px] sm:text-base leading-relaxed prose prose-green max-w-none">
                  <PortableText value={person.fullBio} />
                </div>

                <div className="mt-10 pt-8 border-t border-[#2F3E33]/5">
                  <WhatsAppConsultationLink
                    className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-green px-8 text-sm font-bold text-white hover:bg-green/90 transition-all shadow-lg shadow-green/20"
                  >
          ...
          <WhatsAppConsultationLink
            className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-green text-white hover:bg-green/90 transition-all shadow-md shadow-green/10"
