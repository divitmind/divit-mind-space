...
export function AwarenessPage({ data }: AwarenessPageProps) {
  return (
    <div className="min-h-screen bg-[#FAF9F5]">
      ...
      {/* Section 2 — Why Awareness Matters (verbatim from awareness.txt) */}
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <motion.div
              ...
              className="space-y-5"
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green/10 text-green text-[10px] font-bold uppercase tracking-widest">
                Why This Matters
              </div>
              <h2
                ...
                style={{ fontFamily: "'Cormorant Garamond', 'Georgia', serif" }}
              >
                Awareness is a core pillar of our work
              </h2>
              <div className="space-y-4 text-black/70 font-medium leading-relaxed text-base lg:text-lg max-w-xl">
                {AWARENESS_MISSION_PARAGRAPHS.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              {/* Animated decorative offset — slow rotate + scale breathe */}
              <motion.div
                className="absolute inset-0 bg-purple/10 rounded-[2rem]"
                animate={{
                  rotate: [2, 3.5, 2],
                  x: [8, 12, 8],
                  y: [8, 12, 8],
